  ...
  if(hasSyncBridge()){
    WolfCore.bridgeTask('sync-manual',()=>{
      ...
      onTimeout:()=>{
        ...
        updateSyncPill();if(currentPage==='settings')settings();
      }
    });
  }
...
function loginSync(btn){
  ...
  WolfCore.bridgeTask('auth',()=>WolfSync.login(e,p),{ms:15000,message:'Logowanie trwało zbyt długo.',onTimeout:()=>{syncState='pending';syncTitle='Brak odpowiedzi';updateSyncPill();if(currentPage==='settings')settings()}});syncState='syncing';syncTitle='Logowanie…';updateSyncPill()
}
function registerSync(btn){
  let e=$('#syncEmail')?.value.trim(),p=$('#syncPassword')?.value||'';
  if(!e||p.length<6)return toast('Wpisz e-mail i hasło min. 6 znaków');
  if(btn){btn.classList.add('btn-busy');btn.textContent='Tworzenie…'}
  WolfCore.bridgeTask('auth',()=>WolfSync.register(e,p),{ms:15000,message:'Tworzenie konta trwało zbyt długo.',onTimeout:()=>{syncState='pending';syncTitle='Brak odpowiedzi';updateSyncPill();if(currentPage==='settings')settings()}});syncState='syncing';syncTitle='Tworzenie konta…';updateSyncPill()
}
function logoutSync(btn){
  ...
  WolfCore.bridgeTask('auth',()=>WolfSync.logout(),{ms:12000,message:'Wylogowanie trwało zbyt długo.',onTimeout:()=>{syncState='pending';syncTitle='Brak odpowiedzi';updateSyncPill();if(currentPage==='settings')settings()}});
}
window.wolfSyncAuth=function(logged,email,uid){WolfCore.finish('auth');syncLoggedIn=!!logged;syncEmail=email||'';syncUid=uid||'';syncState=logged?'online':'offline';syncTitle=logged?'Połączono':'Tylko lokalnie';syncDetail=logged?'Synchronizacja jest aktywna.':'Zaloguj się, aby synchronizować.';updateSyncPill();if(logged&&hasSyncBridge()){WolfCore.watch('school-data',20000,'Dane szkoły nie odpowiedziały w czasie.',()=>{if(currentPage==='settings')settings()});WolfSync.requestRemoteData();WolfSync.requestSchoolData();}if(currentPage==='settings')settings()}
window.wolfSyncStatus=function(title,detail,state){if(state&&state!=='syncing')WolfCore.finish('sync-manual');syncTitle=title||'Synchronizacja';syncDetail=detail||'';syncState=state||'offline';updateSyncPill();if(currentPage==='settings')settings()}
window.wolfSyncError=function(message){WolfCore.finish('auth');WolfCore.finish('sync-manual');WolfCore.finish('school-data');syncState='pending';syncTitle='Błąd synchronizacji';syncDetail=message||'Nie udało się połączyć.';updateSyncPill();toast(syncDetail);if(currentPage==='settings')settings()}
window.wolfSyncRemote=function(json,updatedAt){WolfCore.finish('sync-manual');try{let remote=JSON.parse(json),localTime=Number(db?._sync?.updatedAt||0),remoteTime=Number(updatedAt||remote?._sync?.updatedAt||0);if(remoteTime>localTime){applyingRemote=true;db=remote;db._sync=db._sync||{};db._sync.updatedAt=remoteTime;localStorage.setItem(KEY,JSON.stringify(db));applyingRemote=false;render(currentPage||'home');toast('Pobrano nowsze dane z chmury')}syncState='online';syncTitle='Zsynchronizowano';syncDetail='Wszystkie zmiany są aktualne.';updateSyncPill()}catch(e){window.wolfSyncError('Nie udało się odczytać danych z chmury.')}}
