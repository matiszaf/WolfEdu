...
window.wolfSyncAuth=function(logged,email,uid){syncLoggedIn=!!logged;syncEmail=email||'';syncUid=uid||'';syncState=logged?'online':'offline';syncTitle=logged?'Połączono':'Tylko lokalnie';syncDetail=logged?'Synchronizacja jest aktywna.':'Zaloguj się, aby synchronizować.';updateSyncPill();if(logged&&hasSyncBridge()){WolfSync.requestRemoteData();WolfSync.requestSchoolData();}if(currentPage==='settings')settings()}
